...
現在のデプロイ証拠では、このリポジトリは `website-source-v2` が生成するランタイム成果物です。検証済み成果物は GitHub Actions がこのリポジトリの `main` ブランチへ公開し、GitHub Project Pages は `/Private/` サブパスで提供します。別の `website-source` ワークフローはユーザーサイト `NextWeb4.github.io` にのみ公開し、このリポジトリを対象にしません。

## 監査済みインベントリ

次の検証済みデプロイでは、`main` に 660 個のファイルが含まれます。
...
| リポジトリ直下 | 13 | ホーム、About、404、共有資産、`_headers`、`.nojekyll`、4 つの公開保守文書 |
...
1. `website-source-v2` でコンテンツ、テンプレート、検証規則を変更し、実際のビルドおよびテストゲートを実行します。
2. レビュー済みの公開許可リストだけを出力します。
3. このランタイムツリーを原子的に置換し、GitHub Pages から `main` を配信します。
4. 公開済み `/Private/` URL、代表記事、検索、メディア、404 の挙動を確認します。
...
2026-07-22 の監査時点で、リポジトリは公開、未アーカイブ、Pages 有効でした。ソースコミット `2ca9ae2` のデプロイは成功しています。主なリスクはソースと成果物のずれ、直接変更の上書き、大きな静的メディア、生成索引の不整合、非公開ソースだけに属する情報の誤公開です。
