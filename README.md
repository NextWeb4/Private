...
The current deployment evidence shows that this repository is generated output from `website-source-v2`. Its validated artifact is published by GitHub Actions to this repository's `main` branch, and GitHub Project Pages serves it below `/Private/`. The separate `website-source` workflow publishes the user-site runtime to `NextWeb4.github.io`; it does not target this repository.

## Audited inventory

The next validated deployment will contain 660 files on `main`:
...
| Repository root | 13 | Homepage, About and 404 pages, shared assets, `_headers`, `.nojekyll`, and four public maintenance documents |
...
1. Edit content, templates, or validation rules in `website-source-v2` and run its actual build and test gates.
2. Export only the reviewed public allowlist.
3. Replace this runtime tree atomically and let GitHub Pages serve `main`.
4. Verify the deployed `/Private/` URL, representative articles, search, media, and 404 behavior.
...
At the 2026-07-22 audit the repository was public, unarchived, and Pages-enabled; source commit `2ca9ae2` completed deployment successfully. Its principal risks are source/runtime drift, direct edits being overwritten, large static media, generated indexes becoming inconsistent, and accidental publication of material that belongs only in the private source.
