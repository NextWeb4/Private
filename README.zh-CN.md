...
第六轮证据表明本仓库是生成的部署产物，但现有两个私有源码仓库对目标仓库的说明互相冲突。因此，本文不指定未经证实的事实源仓库。修改发布路径前必须先确认当前生效工作流的目标；部署可能替换整个运行树。

## 审计清单

2026-07-22 对 `main` 的递归审计共发现 656 个已提交文件：
...
| 仓库根目录 | 9 | 首页、About、404、共享资源、`_headers` 和 `.nojekyll` |
...
1. 先识别当前生效的私有源码并核验工作流目标；第六轮证据对此存在冲突。
2. 在已确认的源码中编辑内容、模板或校验规则，并运行其真实构建和测试门禁。
3. 只导出审核通过的公开白名单。
4. 原子替换此运行树，并由 GitHub Pages 提供 `main`。
5. 验证已部署的 `/Private/` URL、代表性文章、搜索、媒体和 404 行为。
...
截至 2026-07-22 审计，仓库公开、未归档并已启用 Pages。主要风险是源码与运行产物漂移、直接编辑被覆盖、大体积静态媒体、生成索引失配，以及把只属于私有源码的材料意外发布。
